...
function AddEditModal(props) {
  ...
  const [description, setDescription] = useState(
    !props.newEvent ? props.selectedEvent?.description?.description : ""
  );
  const [startDate, setStartDate] = useState(
    !props.newEvent ? props.selectedEvent.dateStart : props.selectedDate
  );
  const [startTime, setStartTime] = useState(
    !props.newEvent ? props.selectedEvent.dateStart : ""
  );
  const [endTime, setEndTime] = useState(
    !props.newEvent ? props.selectedEvent.dateEnd : ""
  );
  ...
  const [name, setName] = useState(
    !props.newEvent ? props.selectedEvent.summary : ""
  );
  const [allDay, setAllDay] = useState(
    !props.newEvent ? props.selectedEvent.allDay : true
  );
  ...
  const addNewSubject = useCallback(
    (e) => {
      ...
      const newConfigObj = {
        ...config,
        subjectList: [...config.subjectList, newSubject.toUpperCase()],
      };
  ...
  const deleteSubject = useCallback(
    (subject) => {
      const updatedSubjects = config.subjectList.filter(
        (sub) => sub !== subject
      );
  ...
  useEffect(() => {
    ...
    setOrdinalsOfMonth(
      "the " + ordinalString + " " + dayOfWeek + " of the month"
    );
    ...
    <div>
      <Modal.Body>
        <div className="flex, flexCol">
          <div className="flex">
            <DropdownButton id="dropdown-basic-button" title={eventType}>
              <Dropdown.Item onClick={() => setEventType("Class")}>
                Class
              </Dropdown.Item>
              <Dropdown.Item onClick={() => setEventType("Everything Else")}>
                Everything Else
              </Dropdown.Item>
            </DropdownButton>
        ...
        {repeats && (
          <DropdownButton title={rRuleObj.FREQ}>
            <Dropdown.Item
              onClick={(e) => setRRuleObj({ ...rRuleObj, FREQ: "DAILY" })}
            >
              Daily
            </Dropdown.Item>
            <Dropdown.Item
              onClick={(e) =>
                setRRuleObj({
                  ...rRuleObj,
                  FREQ: "WEEKLY",
                  BYDAY: daySelected.substring(0, 2).toUpperCase(),
                })
              }
            >
              Weekly on {daySelected}
            </Dropdown.Item>
            <Dropdown.Item
              onClick={(e) => setRRuleObj({ ...rRuleObj, FREQ: "MONTHLY" })}
            >
              Monthly on {ordinalsOfMonth}
            </Dropdown.Item>
            <Dropdown.Item
              onClick={(e) => setRRuleObj({ ...rRuleObj, FREQ: "YEARLY" })}
            >
      ...
      <Modal.Footer>
        ...
        {displayDeleteOptions && (
          <div className="delete-options">
            <Button onClick={props.deleteFutureEvent}>
              Delete This And All Future Events
            </Button>
            <Button onClick={props.deleteEvent}>Delete All Events</Button>
            <Button onClick={props.deleteSingleEvent}>Delete This Event</Button>
          </div>
        )}
      </Modal.Footer>
    </div>
