...
const filterEvents = (allCurrentEvents, activeMonth, activeYear) => {
  ...
  return allCurrentEvents.filter((e) => {
    ...
    let endMonth = e.endMoment ? moment(e.endMoment).month() + 1 : undefined;
    // const startMonth = e.dateStart ? e.dateStart.getMonth() + 1 : undefined;
    const startMonth = e.startMoment
      ? moment(e.startMoment).month() + 1
      : undefined;
    ...
    if (
      moment(e.endMoment).milliseconds() <
      moment(`${activeYear}-${activeMonth}-01`).milliseconds()
    )
      return false;
    if (
      moment(e.startMoment).milliseconds() >
