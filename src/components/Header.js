...
import addTestEvents from "./devHelperFunctions/addTestEvents";
...
function Header(props) {
  const credentials = useSelector(
    (state) => state.signInStatus.credentialResponse
  );
  ...
  const setUpHICalendar = useCallback(
    async (calendarList) => {
      // console.log('in setUpHICalendar', { calendarList });

      // get the hi calendar
      let hICalendar = calendarList.find(
        (cal) => cal.summary === "Home School Island"
      );
      ...
      console.log({ events });
      let configObject = false;

      // add moment to events
      events.result.items.forEach((e) => {
        if (e.summary === "hiConfig") {
          configObject = { ...defaultConfig, ...JSON.parse(e.description) };
          configObject.id = e.id;
      ...
      dispatch(
        setEvents(events.result.items.filter((e) => e.summary !== "hiConfig"))
      );
      ...
      if (!configObject) {
        ...
        configObject.saved = true;
        // console.log('makeConfig return', { configObject })
      }
      console.log("Header", { configObject });
  ...
  const getCalendars = useCallback(() => {
    gapi.client.calendar.calendarList.list().then(function (response) {
      ...
      let calendarList = _.sortBy(
        response.result.items,
        (item) => item.summaryOverride || item.summary
      );
    ...
    gapi.load("client", async () => {
      ...
      let tokenClient = window.google.accounts.oauth2.initTokenClient({
        ...
        callback: (tokenResponse) => {
          ...
          const isSignedIn = window.google.accounts.oauth2.hasGrantedAllScopes(
            tokenResponse,
            SCOPES
          );
          console.log(`gapi hasGrantedAllScopesGoogle`, {
            SCOPES,
            isSignedIn,
            tokenClient,
          });
      ...
      const isSignedIn = hasGrantedAllScopesGoogle(credentials, SCOPES);
    ...
    <div id="header">
      ...
      {window.location.hostname === "localhost" && (
        <div>
          <button
            onClick={async () => {
              await GAPI.deleteCalendar(hICalendar.id);
          ...
          <button
            onClick={async () => {
              console.log("clicked add test events");
              const addedEvents = await addTestEvents({ hICalendar });
              dispatch(setEvents([...events, ...addedEvents]));
            }}
          >
            Add Test Events
          </button>
